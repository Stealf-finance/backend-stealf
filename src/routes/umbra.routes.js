 ...
 */
router.post('/transfer/simple-private', async (req, res) => {
    try {
 ...
 */
router.post('/deposit/confidential', async (req, res) => {
    try {
 ...
 */
router.get('/deposits/claimable', async (req, res) => {
    try {
        ...
        const deposits = await depositService.getClaimableDeposits(userId);
        res.json({
            success: true,
            deposits,
 ...
 */
router.get('/deposits/claimed', async (req, res) => {
    try {
        const { userId } = req.query;
        if (!userId) {
            return res.status(400).json({
        ...
        const deposits = await claimService.getClaimedDeposits(userId);
        res.json({
            success: true,
            deposits,
            count: deposits.length
...
router.get('/balance', async (req, res) => {
    try {
        ...
        const balanceByMint = claimableDeposits.reduce((acc, deposit) => {
            if (!acc[deposit.mint]) {
                acc[deposit.mint] = {
                    mint: deposit.mint,
        ...
        const balances = Object.values(balanceByMint).map((balance) => ({
            mint: balance.mint,
