...
import { convertAccountOffsetToTransactionInput, convertArciumX25519NonceToTransactionInput, convertComputationOffsetToTransactionInput, convertRescueCiphertextToTransactionInput, convertSha3HashToTransactionInput, } from '@/types';
import { generateRandomComputationOffset, getArciumAccounts } from '@/utils/arcium';
import { generateRandomArciumCommissionFeesAccountOffset, generateRandomFeesConfigurationAccountOffset, generateRandomRelayerFeesPoolOffset, } from '@/utils/miscellaneous';
import { getFeesConfigurationPda, getArciumCommissionFeesPoolPda, getRelayerFeesPoolPda, } from '@/utils/pda-generators';
/**
 ...
 * ```
 */
export async function buildNewTokenTransferSolMxeInstruction(txAccounts, txArgs) {
    try {
        const INSTRUCTION_SEED = BigInt(11);
        const computationOffset = generateRandomComputationOffset();
        const relayerFeesPoolOffset = generateRandomRelayerFeesPoolOffset(INSTRUCTION_SEED, WSOL_MINT_ADDRESS);
        const arciumCommissionFeesAccountOffset = generateRandomArciumCommissionFeesAccountOffset(INSTRUCTION_SEED, WSOL_MINT_ADDRESS);
        const feesConfigurationAccountOffset = generateRandomFeesConfigurationAccountOffset(INSTRUCTION_SEED, WSOL_MINT_ADDRESS);
        const relayerFeesPoolAccount = getRelayerFeesPoolPda(txAccounts.relayer, INSTRUCTION_SEED, WSOL_MINT_ADDRESS, relayerFeesPoolOffset);
        const arciumCommissionFeesAccount = getArciumCommissionFeesPoolPda(WSOL_MINT_ADDRESS, INSTRUCTION_SEED, arciumCommissionFeesAccountOffset);
        const feesConfigurationAccount = getFeesConfigurationPda(INSTRUCTION_SEED, WSOL_MINT_ADDRESS, feesConfigurationAccountOffset);
        const { computationAccount, compDefAccount, clusterAccount, mxeAccount, mempoolAccount, executingPool, } = getArciumAccounts(computationOffset, 'new_token_transfer_sol_mxe');
        const ixBuilder = program.methods
            .newTokenTransferSolMxe(convertComputationOffsetToTransactionInput(computationOffset), convertAccountOffsetToTransactionInput(relayerFeesPoolOffset), convertAccountOffsetToTransactionInput(arciumCommissionFeesAccountOffset), convertAccountOffsetToTransactionInput(feesConfigurationAccountOffset), convertRescueCiphertextToTransactionInput(txArgs.transferAmountCiphertext), convertArciumX25519NonceToTransactionInput(txArgs.transferAmountNonce), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
        ...
        if (!instruction) {
            throw new InstructionBuildingError('Instruction builder returned null or undefined');
        }
        return instruction;
    }
    catch (error) {
        ...
        throw new InstructionBuildingError(`Failed to build new token transfer SOL MXE instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildExistingTokenTransferSolMxeInstruction(txAccounts, txArgs) {
    try {
        const INSTRUCTION_SEED = BigInt(12);
        const computationOffset = generateRandomComputationOffset();
        const relayerFeesPoolOffset = generateRandomRelayerFeesPoolOffset(INSTRUCTION_SEED, WSOL_MINT_ADDRESS);
        const arciumCommissionFeesAccountOffset = generateRandomArciumCommissionFeesAccountOffset(INSTRUCTION_SEED, WSOL_MINT_ADDRESS);
        const feesConfigurationAccountOffset = generateRandomFeesConfigurationAccountOffset(INSTRUCTION_SEED, WSOL_MINT_ADDRESS);
        ...
        const arciumCommissionFeesAccount = getArciumCommissionFeesPoolPda(WSOL_MINT_ADDRESS, INSTRUCTION_SEED, arciumCommissionFeesAccountOffset);
        const feesConfigurationAccount = getFeesConfigurationPda(INSTRUCTION_SEED, WSOL_MINT_ADDRESS, feesConfigurationAccountOffset);
        const { computationAccount, compDefAccount, clusterAccount, mxeAccount, mempoolAccount, executingPool, } = getArciumAccounts(computationOffset, 'existing_token_transfer_sol_mxe');
        const ixBuilder = program.methods
            .existingTokenTransferSolMxe(convertComputationOffsetToTransactionInput(computationOffset), convertAccountOffsetToTransactionInput(relayerFeesPoolOffset), convertAccountOffsetToTransactionInput(arciumCommissionFeesAccountOffset), convertAccountOffsetToTransactionInput(feesConfigurationAccountOffset), convertRescueCiphertextToTransactionInput(txArgs.transferAmountCiphertext), convertArciumX25519NonceToTransactionInput(txArgs.transferAmountNonce), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            relayer: txAccounts.relayer,
            receiverAddress: txAccounts.receiver,
            arciumSenderSigner: txAccounts.arciumSenderSigner,
        ...
        throw new InstructionBuildingError(`Failed to build existing token transfer SOL MXE instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildNewTokenTransferSolSharedInstruction(txAccounts, txArgs) {
    try {
        const INSTRUCTION_SEED = BigInt(13);
        const computationOffset = generateRandomComputationOffset();
        const relayerFeesPoolOffset = generateRandomRelayerFeesPoolOffset(INSTRUCTION_SEED, WSOL_MINT_ADDRESS);
        const arciumCommissionFeesAccountOffset = generateRandomArciumCommissionFeesAccountOffset(INSTRUCTION_SEED, WSOL_MINT_ADDRESS);
        const feesConfigurationAccountOffset = generateRandomFeesConfigurationAccountOffset(INSTRUCTION_SEED, WSOL_MINT_ADDRESS);
        ...
        const arciumCommissionFeesAccount = getArciumCommissionFeesPoolPda(WSOL_MINT_ADDRESS, INSTRUCTION_SEED, arciumCommissionFeesAccountOffset);
        const feesConfigurationAccount = getFeesConfigurationPda(INSTRUCTION_SEED, WSOL_MINT_ADDRESS, feesConfigurationAccountOffset);
        const { computationAccount, compDefAccount, clusterAccount, mxeAccount, mempoolAccount, executingPool, } = getArciumAccounts(computationOffset, 'new_token_transfer_sol_shared');
        const ixBuilder = program.methods
            .newTokenTransferSolShared(convertComputationOffsetToTransactionInput(computationOffset), convertAccountOffsetToTransactionInput(relayerFeesPoolOffset), convertAccountOffsetToTransactionInput(arciumCommissionFeesAccountOffset), convertAccountOffsetToTransactionInput(feesConfigurationAccountOffset), convertRescueCiphertextToTransactionInput(txArgs.transferAmountCiphertext), convertArciumX25519NonceToTransactionInput(txArgs.transferAmountNonce), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            relayer: txAccounts.relayer,
            receiverAddress: txAccounts.receiver,
            arciumSenderSigner: txAccounts.arciumSenderSigner,
        ...
        throw new InstructionBuildingError(`Failed to build new token transfer SOL shared instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildExistingTokenTransferSolSharedInstruction(txAccounts, txArgs) {
    try {
        const INSTRUCTION_SEED = BigInt(14);
        const computationOffset = generateRandomComputationOffset();
        const relayerFeesPoolOffset = generateRandomRelayerFeesPoolOffset(INSTRUCTION_SEED, WSOL_MINT_ADDRESS);
        const arciumCommissionFeesAccountOffset = generateRandomArciumCommissionFeesAccountOffset(INSTRUCTION_SEED, WSOL_MINT_ADDRESS);
        const feesConfigurationAccountOffset = generateRandomFeesConfigurationAccountOffset(INSTRUCTION_SEED, WSOL_MINT_ADDRESS);
        ...
        const arciumCommissionFeesAccount = getArciumCommissionFeesPoolPda(WSOL_MINT_ADDRESS, INSTRUCTION_SEED, arciumCommissionFeesAccountOffset);
        const feesConfigurationAccount = getFeesConfigurationPda(INSTRUCTION_SEED, WSOL_MINT_ADDRESS, feesConfigurationAccountOffset);
        const { computationAccount, compDefAccount, clusterAccount, mxeAccount, mempoolAccount, executingPool, } = getArciumAccounts(computationOffset, 'existing_token_transfer_sol_shared');
        const ixBuilder = program.methods
            .existingTokenTransferSolShared(convertComputationOffsetToTransactionInput(computationOffset), convertAccountOffsetToTransactionInput(relayerFeesPoolOffset), convertAccountOffsetToTransactionInput(arciumCommissionFeesAccountOffset), convertAccountOffsetToTransactionInput(feesConfigurationAccountOffset), convertRescueCiphertextToTransactionInput(txArgs.transferAmountCiphertext), convertArciumX25519NonceToTransactionInput(txArgs.transferAmountNonce), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            relayer: txAccounts.relayer,
            receiverAddress: txAccounts.receiver,
            arciumSenderSigner: txAccounts.arciumSenderSigner,
        ...
        throw new InstructionBuildingError(`Failed to build existing token transfer SOL shared instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildNewTokenTransferSplMxeInstruction(txAccounts, txArgs) {
    try {
        const INSTRUCTION_SEED = BigInt(15);
        const computationOffset = generateRandomComputationOffset();
        const relayerFeesPoolOffset = generateRandomRelayerFeesPoolOffset(INSTRUCTION_SEED, txAccounts.mint);
        const arciumCommissionFeesAccountOffset = generateRandomArciumCommissionFeesAccountOffset(INSTRUCTION_SEED, txAccounts.mint);
        const feesConfigurationAccountOffset = generateRandomFeesConfigurationAccountOffset(INSTRUCTION_SEED, txAccounts.mint);
        const relayerFeesPoolAccount = getRelayerFeesPoolPda(txAccounts.relayer, INSTRUCTION_SEED, txAccounts.mint, relayerFeesPoolOffset);
        const arciumCommissionFeesAccount = getArciumCommissionFeesPoolPda(txAccounts.mint, INSTRUCTION_SEED, arciumCommissionFeesAccountOffset);
        const feesConfigurationAccount = getFeesConfigurationPda(INSTRUCTION_SEED, txAccounts.mint, feesConfigurationAccountOffset);
        const { computationAccount, compDefAccount, clusterAccount, mxeAccount, mempoolAccount, executingPool, } = getArciumAccounts(computationOffset, 'new_token_transfer_spl_mxe');
        const ixBuilder = program.methods
            .newTokenTransferSplMxe(convertComputationOffsetToTransactionInput(computationOffset), convertAccountOffsetToTransactionInput(relayerFeesPoolOffset), convertAccountOffsetToTransactionInput(arciumCommissionFeesAccountOffset), convertAccountOffsetToTransactionInput(feesConfigurationAccountOffset), convertRescueCiphertextToTransactionInput(txArgs.transferAmountCiphertext), convertArciumX25519NonceToTransactionInput(txArgs.transferAmountNonce), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            relayer: txAccounts.relayer,
            receiverAddress: txAccounts.receiver,
            arciumSenderSigner: txAccounts.arciumSenderSigner,
        ...
        throw new InstructionBuildingError(`Failed to build new token transfer SPL MXE instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildExistingTokenTransferSplMxeInstruction(txAccounts, txArgs) {
    try {
        const INSTRUCTION_SEED = BigInt(16);
        const computationOffset = generateRandomComputationOffset();
        const relayerFeesPoolOffset = generateRandomRelayerFeesPoolOffset(INSTRUCTION_SEED, txAccounts.mint);
        const arciumCommissionFeesAccountOffset = generateRandomArciumCommissionFeesAccountOffset(INSTRUCTION_SEED, txAccounts.mint);
        const feesConfigurationAccountOffset = generateRandomFeesConfigurationAccountOffset(INSTRUCTION_SEED, txAccounts.mint);
        ...
        const arciumCommissionFeesAccount = getArciumCommissionFeesPoolPda(txAccounts.mint, INSTRUCTION_SEED, arciumCommissionFeesAccountOffset);
        const feesConfigurationAccount = getFeesConfigurationPda(INSTRUCTION_SEED, txAccounts.mint, feesConfigurationAccountOffset);
        const { computationAccount, compDefAccount, clusterAccount, mxeAccount, mempoolAccount, executingPool, } = getArciumAccounts(computationOffset, 'existing_token_transfer_spl_mxe');
        const ixBuilder = program.methods
            .existingTokenTransferSplMxe(convertComputationOffsetToTransactionInput(computationOffset), convertAccountOffsetToTransactionInput(relayerFeesPoolOffset), convertAccountOffsetToTransactionInput(arciumCommissionFeesAccountOffset), convertAccountOffsetToTransactionInput(feesConfigurationAccountOffset), convertRescueCiphertextToTransactionInput(txArgs.transferAmountCiphertext), convertArciumX25519NonceToTransactionInput(txArgs.transferAmountNonce), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            relayer: txAccounts.relayer,
            receiverAddress: txAccounts.receiver,
            arciumSenderSigner: txAccounts.arciumSenderSigner,
        ...
        throw new InstructionBuildingError(`Failed to build existing token transfer SPL MXE instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildNewTokenTransferSplSharedInstruction(txAccounts, txArgs) {
    try {
        const INSTRUCTION_SEED = BigInt(17);
        const computationOffset = generateRandomComputationOffset();
        const relayerFeesPoolOffset = generateRandomRelayerFeesPoolOffset(INSTRUCTION_SEED, txAccounts.mint);
        const arciumCommissionFeesAccountOffset = generateRandomArciumCommissionFeesAccountOffset(INSTRUCTION_SEED, txAccounts.mint);
        const feesConfigurationAccountOffset = generateRandomFeesConfigurationAccountOffset(INSTRUCTION_SEED, txAccounts.mint);
        ...
        const arciumCommissionFeesAccount = getArciumCommissionFeesPoolPda(txAccounts.mint, INSTRUCTION_SEED, arciumCommissionFeesAccountOffset);
        const feesConfigurationAccount = getFeesConfigurationPda(INSTRUCTION_SEED, txAccounts.mint, feesConfigurationAccountOffset);
        const { computationAccount, compDefAccount, clusterAccount, mxeAccount, mempoolAccount, executingPool, } = getArciumAccounts(computationOffset, 'new_token_transfer_spl_shared');
        const ixBuilder = program.methods
            .newTokenTransferSplShared(convertComputationOffsetToTransactionInput(computationOffset), convertAccountOffsetToTransactionInput(relayerFeesPoolOffset), convertAccountOffsetToTransactionInput(arciumCommissionFeesAccountOffset), convertAccountOffsetToTransactionInput(feesConfigurationAccountOffset), convertRescueCiphertextToTransactionInput(txArgs.transferAmountCiphertext), convertArciumX25519NonceToTransactionInput(txArgs.transferAmountNonce), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            relayer: txAccounts.relayer,
            receiverAddress: txAccounts.receiver,
            arciumSenderSigner: txAccounts.arciumSenderSigner,
        ...
        throw new InstructionBuildingError(`Failed to build new token transfer SPL shared instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildExistingTokenTransferSplSharedInstruction(txAccounts, txArgs) {
    try {
        const INSTRUCTION_SEED = BigInt(18);
        const computationOffset = generateRandomComputationOffset();
        const relayerFeesPoolOffset = generateRandomRelayerFeesPoolOffset(INSTRUCTION_SEED, txAccounts.mint);
        const arciumCommissionFeesAccountOffset = generateRandomArciumCommissionFeesAccountOffset(INSTRUCTION_SEED, txAccounts.mint);
        const feesConfigurationAccountOffset = generateRandomFeesConfigurationAccountOffset(INSTRUCTION_SEED, txAccounts.mint);
        ...
        const arciumCommissionFeesAccount = getArciumCommissionFeesPoolPda(txAccounts.mint, INSTRUCTION_SEED, arciumCommissionFeesAccountOffset);
        const feesConfigurationAccount = getFeesConfigurationPda(INSTRUCTION_SEED, txAccounts.mint, feesConfigurationAccountOffset);
        const { computationAccount, compDefAccount, clusterAccount, mxeAccount, mempoolAccount, executingPool, } = getArciumAccounts(computationOffset, 'existing_token_transfer_spl_shared');
        const ixBuilder = program.methods
            .existingTokenTransferSplShared(convertComputationOffsetToTransactionInput(computationOffset), convertAccountOffsetToTransactionInput(relayerFeesPoolOffset), convertAccountOffsetToTransactionInput(arciumCommissionFeesAccountOffset), convertAccountOffsetToTransactionInput(feesConfigurationAccountOffset), convertRescueCiphertextToTransactionInput(txArgs.transferAmountCiphertext), convertArciumX25519NonceToTransactionInput(txArgs.transferAmountNonce), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            relayer: txAccounts.relayer,
            receiverAddress: txAccounts.receiver,
            arciumSenderSigner: txAccounts.arciumSenderSigner,
        ...
        throw new InstructionBuildingError(`Failed to build existing token transfer SPL shared instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
