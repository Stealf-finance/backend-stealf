...
import { convertAccountOffsetToTransactionInput, convertArciumX25519NonceToTransactionInput, convertArciumX25519PublicKeyToTransactionInput, convertComputationOffsetToTransactionInput, convertGroth16ProofABeBytesToTransactionInput, convertGroth16ProofBBeBytesToTransactionInput, convertGroth16ProofCBeBytesToTransactionInput, convertPoseidonHashToTransactionInput, convertRescueCiphertextToTransactionInput, convertSha3HashToTransactionInput, } from '@/types';
import { generateRandomComputationOffset, getArciumAccounts } from '@/utils/arcium';
import { generateRandomFeesConfigurationAccountOffset, generateRandomRelayerFeesPoolOffset, } from '@/utils/miscellaneous';
 ...
 * ```
 */
export async function buildUpdateMasterViewingKeyInstruction(txAccounts, txArgs) {
    try {
        const computationOffset = generateRandomComputationOffset();
        const { computationAccount, compDefAccount, clusterAccount, mxeAccount, mempoolAccount, executingPool, } = getArciumAccounts(computationOffset, 'update_master_viewing_key');
        const ixBuilder = program.methods
            .updateMasterViewingKey(convertComputationOffsetToTransactionInput(computationOffset), convertRescueCiphertextToTransactionInput(txArgs.masterViewingKeyCiphertext), convertRescueCiphertextToTransactionInput(txArgs.masterViewingKeyBlindingFactor), convertArciumX25519NonceToTransactionInput(txArgs.masterViewingKeyNonce), convertSha3HashToTransactionInput(txArgs.masterViewingKeyShaCommitment), convertPoseidonHashToTransactionInput(txArgs.masterViewingKeyHash), convertGroth16ProofABeBytesToTransactionInput(txArgs.proofA), convertGroth16ProofBBeBytesToTransactionInput(txArgs.proofB), convertGroth16ProofCBeBytesToTransactionInput(txArgs.proofC), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
        ...
        throw new InstructionBuildingError(`Failed to build update master viewing key instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildConvertUserAccountFromMxeToSharedInstruction(txAccounts, txArgs) {
    try {
        const ixBuilder = program.methods
            .convertUserAccountFromMxeToShared(convertArciumX25519PublicKeyToTransactionInput(txArgs.x25519PublicKey), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            arciumSigner: txAccounts.arciumSigner,
        });
        const instruction = await ixBuilder.instruction();
        ...
        throw new InstructionBuildingError(`Failed to build convert user account from MXE to shared instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildConvertTokenAccountFromMxeToSharedInstruction(txAccounts, txArgs) {
    try {
        const computationOffset = generateRandomComputationOffset();
        const { computationAccount, compDefAccount, clusterAccount, mxeAccount, mempoolAccount, executingPool, } = getArciumAccounts(computationOffset, 'convert_token_account_from_mxe_to_shared');
        const ixBuilder = program.methods
            .convertTokenAccountFromMxeToShared(convertComputationOffsetToTransactionInput(computationOffset), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            payer: txAccounts.payer,
        ...
        throw new InstructionBuildingError(`Failed to build convert token account from MXE to shared instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildConvertTokenAccountFromMxeToSharedSolThroughRelayerInstruction(txAccounts, txArgs) {
    try {
        const INSTRUCTION_SEED = BigInt(37);
        const MINT_ADDRESS = new PublicKey('So11111111111111111111111111111111111111112');
        const computationOffset = generateRandomComputationOffset();
        const relayerFeesPoolOffset = generateRandomRelayerFeesPoolOffset(INSTRUCTION_SEED, MINT_ADDRESS);
        const feesConfigurationAccountOffset = generateRandomFeesConfigurationAccountOffset(INSTRUCTION_SEED, MINT_ADDRESS);
        const relayerFeesPoolAccount = getRelayerFeesPoolPda(txAccounts.relayer, INSTRUCTION_SEED, MINT_ADDRESS, relayerFeesPoolOffset);
        const feesConfigurationAccount = getFeesConfigurationPda(INSTRUCTION_SEED, MINT_ADDRESS, feesConfigurationAccountOffset);
        const { computationAccount, compDefAccount, clusterAccount, mxeAccount, mempoolAccount, executingPool, } = getArciumAccounts(computationOffset, 'convert_token_account_from_mxe_to_shared_sol_through_relayer');
        const ixBuilder = program.methods
            .convertTokenAccountFromMxeToSharedSolThroughRelayer(convertComputationOffsetToTransactionInput(computationOffset), convertAccountOffsetToTransactionInput(relayerFeesPoolOffset), convertAccountOffsetToTransactionInput(feesConfigurationAccountOffset), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
        ...
        throw new InstructionBuildingError(`Failed to build convert token account from MXE to shared SOL through relayer instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildConvertTokenAccountFromMxeToSharedSplThroughRelayer(txAccounts, txArgs) {
    try {
        // Validate that mint address is not WSOL
        if (txAccounts.mint.equals(WSOL_MINT_ADDRESS)) {
            throw new InstructionBuildingError('Mint address cannot be WSOL. Use buildConvertTokenAccountFromMxeToSharedSolThroughRelayerInstruction for SOL conversions.');
        }
        const INSTRUCTION_SEED = BigInt(38);
        const computationOffset = generateRandomComputationOffset();
        ...
        const { computationAccount, compDefAccount, clusterAccount, mxeAccount, mempoolAccount, executingPool, } = getArciumAccounts(computationOffset, 'convert_token_account_from_mxe_to_shared_spl_through_relayer');
        const ixBuilder = program.methods
            .convertTokenAccountFromMxeToSharedSplThroughRelayer(convertComputationOffsetToTransactionInput(computationOffset), convertAccountOffsetToTransactionInput(relayerFeesPoolOffset), convertAccountOffsetToTransactionInput(feesConfigurationAccountOffset), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            relayer: txAccounts.relayer,
            arciumSigner: txAccounts.arciumSigner,
            relayerFeesPool: relayerFeesPoolAccount,
        ...
        throw new InstructionBuildingError(`Failed to build convert token account from MXE to shared SPL through relayer instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
