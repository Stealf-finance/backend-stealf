...
import { convertAccountOffsetToTransactionInput, convertAmountToTransactionInput, convertBasisPointsToTransactionInput, convertBooleanToTransactionInput, convertInstructionSeedToTransactionInput, convertNumberOfTransactionsToTransactionInput, convertRiskThresholdToTransactionInput, convertSha3HashToTransactionInput, convertSolanaAddressToTransactionInput, } from '@/types';
 ...
 * ```
 */
export async function buildInitialiseFeesConfigurationInstruction(txAccounts, txArgs) {
    try {
        const feesConfigurationAccount = getFeesConfigurationPda(txArgs.instructionSeed, txAccounts.mint, txArgs.accountOffset);
        const ixBuilder = program.methods
            .initialiseFeesConfiguration(convertInstructionSeedToTransactionInput(txArgs.instructionSeed), convertAccountOffsetToTransactionInput(txArgs.accountOffset), convertAmountToTransactionInput(txArgs.relayerFees), convertAmountToTransactionInput(txArgs.commissionFeesLowerBound), convertAmountToTransactionInput(txArgs.commissionFeesUpperBound), convertBasisPointsToTransactionInput(txArgs.commissionFees), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
        ...
        throw new InstructionBuildingError(`Failed to build initialise fees configuration instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildModifyFeesConfigurationInstruction(txAccounts, txArgs) {
    try {
        const feesConfigurationAccount = getFeesConfigurationPda(txArgs.instructionSeed, txAccounts.mint, txArgs.accountOffset);
        const ixBuilder = program.methods
            .modifyFeesConfiguration(convertInstructionSeedToTransactionInput(txArgs.instructionSeed), convertAccountOffsetToTransactionInput(txArgs.accountOffset), convertAmountToTransactionInput(txArgs.relayerFees), convertAmountToTransactionInput(txArgs.commissionFeesLowerBound), convertAmountToTransactionInput(txArgs.commissionFeesUpperBound), convertBasisPointsToTransactionInput(txArgs.commissionFees), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            feesConfiguration: feesConfigurationAccount,
            mint: txAccounts.mint,
            signer: txAccounts.signer,
        ...
        throw new InstructionBuildingError(`Failed to build modify fees configuration instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildDeleteFeesConfigurationInstruction(txAccounts, txArgs) {
    try {
        const feesConfigurationAccount = getFeesConfigurationPda(txArgs.instructionSeed, txAccounts.mint, txArgs.accountOffset);
        const ixBuilder = program.methods
            .deleteFeesConfiguration(convertInstructionSeedToTransactionInput(txArgs.instructionSeed), convertAccountOffsetToTransactionInput(txArgs.accountOffset), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            feesConfiguration: feesConfigurationAccount,
            mint: txAccounts.mint,
            signer: txAccounts.signer,
        ...
        throw new InstructionBuildingError(`Failed to build delete fees configuration instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildFreezeMixerPoolInstruction(txAccounts, txArgs) {
    try {
        const ixBuilder = program.methods
        ...
        throw new InstructionBuildingError(`Failed to build freeze mixer pool instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildInitialiseMixerPoolInstruction(txAccounts, txArgs) {
    try {
        const ixBuilder = program.methods
        ...
        throw new InstructionBuildingError(`Failed to build initialise mixer pool instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildInitialiseProgramInformationInstruction(txAccounts, txArgs) {
    try {
        const ixBuilder = program.methods
            .initialiseProgramInformation(convertNumberOfTransactionsToTransactionInput(txArgs.minimumNumberOfTransactions), convertRiskThresholdToTransactionInput(txArgs.riskThreshold), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            signer: txAccounts.signer,
        });
        const instruction = await ixBuilder.instruction();
        ...
        throw new InstructionBuildingError(`Failed to build initialise program information instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildModifyProgramInformationInstruction(txAccounts, txArgs) {
    try {
        const ixBuilder = program.methods
            .modifyProgramInformation(convertBooleanToTransactionInput(txArgs.enableProgram), convertAmountToTransactionInput(txArgs.minimumSolBalanceRequired), convertNumberOfTransactionsToTransactionInput(txArgs.minimumNumberOfTransactionsBeforeWithdrawal), convertRiskThresholdToTransactionInput(txArgs.riskThreshold), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            signer: txAccounts.signer,
        });
        const instruction = await ixBuilder.instruction();
        ...
        throw new InstructionBuildingError(`Failed to build modify program information instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildInitialiseMasterWalletSpecifierInstruction(txAccounts, txArgs) {
    try {
        const ixBuilder = program.methods
            .initialiseMasterWalletSpecifier(convertSolanaAddressToTransactionInput(txArgs.allowedAddress))
            .accountsPartial({
            signer: txAccounts.signer,
        });
        const instruction = await ixBuilder.instruction();
        ...
        throw new InstructionBuildingError(`Failed to build initialise master wallet specifier instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildInitialiseWalletSpecifierInstruction(txAccounts, txArgs) {
    try {
        const walletSpecifierAccount = getWalletSpecifierPda(txArgs.instructionSeed);
        const ixBuilder = program.methods
            .initialiseWalletSpecifier(convertInstructionSeedToTransactionInput(txArgs.instructionSeed), convertSolanaAddressToTransactionInput(txArgs.allowedAddress), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            signer: txAccounts.signer,
        ...
        throw new InstructionBuildingError(`Failed to build initialise wallet specifier instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildModifyWalletSpecifierInstruction(txAccounts, txArgs) {
    try {
        const walletSpecifierAccount = getWalletSpecifierPda(txArgs.instructionSeed);
        const ixBuilder = program.methods
            .modifyWalletSpecifier(convertInstructionSeedToTransactionInput(txArgs.instructionSeed), convertSolanaAddressToTransactionInput(txArgs.allowedAddress), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            signer: txAccounts.signer,
            walletSpecifier: walletSpecifierAccount,
        });
        ...
        throw new InstructionBuildingError(`Failed to build modify wallet specifier instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildInitialiseZkMerkleTreeInstruction(txAccounts, txArgs) {
    try {
        const ixBuilder = program.methods
            .initialiseZkMerkleTree(convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            mint: txAccounts.mint,
            signer: txAccounts.signer,
        });
        ...
        throw new InstructionBuildingError(`Failed to build initialise ZK Merkle tree instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
