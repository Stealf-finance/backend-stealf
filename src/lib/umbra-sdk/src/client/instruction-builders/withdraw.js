...
import { convertAccountOffsetToTransactionInput, convertRescueCiphertextToTransactionInput, convertSha3HashToTransactionInput, convertTimeToTransactionInput, convertPoseidonHashToTransactionInput, convertArciumX25519NonceToTransactionInput, convertComputationOffsetToTransactionInput, convertGroth16ProofABeBytesToTransactionInput, convertGroth16ProofBBeBytesToTransactionInput, convertGroth16ProofCBeBytesToTransactionInput, convertAmountToTransactionInput, convertArciumX25519PublicKeyToTransactionInput, } from '@/types';
...
import { getArciumCommissionFeesPoolPda, getFeesConfigurationPda, getPublicCommissionFeesPoolPda, getRelayerFeesPoolPda, } from '@/utils/pda-generators';
/**
 ...
 * ```
 */
export async function buildWithdrawIntoMixerPoolSolInstruction(txAccounts, txArgs) {
    try {
        const INSTRUCTION_SEED = BigInt(21);
        ...
        const commissionFeesAccountOffset = generateRandomArciumCommissionFeesAccountOffset(INSTRUCTION_SEED, WSOL_MINT_ADDRESS);
        ...
        const commissionFeesPoolAccount = getArciumCommissionFeesPoolPda(WSOL_MINT_ADDRESS, INSTRUCTION_SEED, commissionFeesAccountOffset);
        const feesConfigurationAccount = getFeesConfigurationPda(INSTRUCTION_SEED, WSOL_MINT_ADDRESS, feesConfigurationAccountOffset);
        const { computationAccount, compDefAccount, clusterAccount, mxeAccount, mempoolAccount, executingPool, } = getArciumAccounts(computationOffset, 'withdrawIntoMixerPoolSol');
        const ixBuilder = program.methods
            .withdrawIntoMixerPoolSol(convertComputationOffsetToTransactionInput(computationOffset), convertAccountOffsetToTransactionInput(relayerFeesPoolOffset), convertAccountOffsetToTransactionInput(commissionFeesAccountOffset), convertAccountOffsetToTransactionInput(feesConfigurationAccountOffset), convertRescueCiphertextToTransactionInput(txArgs.withdrawalAmountCiphertext), convertRescueCiphertextToTransactionInput(txArgs.withdrawalAmountBlindingFactor), convertArciumX25519NonceToTransactionInput(txArgs.withdrawalAmountNonce), convertSha3HashToTransactionInput(txArgs.withdrawalAmountCommitment), convertGroth16ProofABeBytesToTransactionInput(txArgs.groth16ProofA), convertGroth16ProofBBeBytesToTransactionInput(txArgs.groth16ProofB), convertGroth16ProofCBeBytesToTransactionInput(txArgs.groth16ProofC), convertTimeToTransactionInput(txArgs.time), convertPoseidonHashToTransactionInput(txArgs.linkerHash), convertPoseidonHashToTransactionInput(txArgs.depositCommitment), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
        ...
        throw new InstructionBuildingError(`Failed to build withdraw into mixer pool SOL instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildWithdrawIntoMixerPoolSplInstruction(txAccounts, txArgs) {
    try {
        const INSTRUCTION_SEED = BigInt(22);
        const computationOffset = generateRandomComputationOffset();
        ...
        const commissionFeesPoolAccount = getArciumCommissionFeesPoolPda(txAccounts.mint, INSTRUCTION_SEED, commissionFeesAccountOffset);
        const feesConfigurationAccount = getFeesConfigurationPda(INSTRUCTION_SEED, txAccounts.mint, feesConfigurationAccountOffset);
        const { computationAccount, compDefAccount, clusterAccount, mxeAccount, mempoolAccount, executingPool, } = getArciumAccounts(computationOffset, 'withdrawIntoMixerPoolSpl');
        const ixBuilder = program.methods
            .withdrawIntoMixerPoolSpl(convertComputationOffsetToTransactionInput(computationOffset), convertAccountOffsetToTransactionInput(relayerFeesPoolOffset), convertAccountOffsetToTransactionInput(commissionFeesAccountOffset), convertAccountOffsetToTransactionInput(feesConfigurationAccountOffset), convertRescueCiphertextToTransactionInput(txArgs.withdrawalAmountCiphertext), convertRescueCiphertextToTransactionInput(txArgs.withdrawalAmountBlindingFactor), convertArciumX25519NonceToTransactionInput(txArgs.withdrawalAmountNonce), convertSha3HashToTransactionInput(txArgs.withdrawalAmountCommitment), convertGroth16ProofABeBytesToTransactionInput(txArgs.groth16ProofA), convertGroth16ProofBBeBytesToTransactionInput(txArgs.groth16ProofB), convertGroth16ProofCBeBytesToTransactionInput(txArgs.groth16ProofC), convertTimeToTransactionInput(txArgs.time), convertPoseidonHashToTransactionInput(txArgs.linkerHash), convertPoseidonHashToTransactionInput(txArgs.depositCommitment), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            relayer: txAccounts.relayer,
            relayerFeesPool: relayerFeesPoolAccount,
            commissionFeesPool: commissionFeesPoolAccount,
        ...
        throw new InstructionBuildingError(`Failed to build withdraw into mixer pool SPL instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildWithdrawFromMixerMxeInstruction(txAccounts, txArgs) {
    try {
        const INSTRUCTION_SEED = BigInt(23);
        const computationOffset = generateRandomComputationOffset();
        const publicCommissionFeesAccountOffset = generateRandomPublicCommissionFeesAccountOffset(INSTRUCTION_SEED, txAccounts.mint);
        const feesConfigurationAccountOffset = generateRandomFeesConfigurationAccountOffset(INSTRUCTION_SEED, txAccounts.mint);
        const publicCommissionFeesAccount = getPublicCommissionFeesPoolPda(txAccounts.mint, INSTRUCTION_SEED, publicCommissionFeesAccountOffset);
        const feesConfigurationAccount = getFeesConfigurationPda(INSTRUCTION_SEED, txAccounts.mint, feesConfigurationAccountOffset);
        const { computationAccount, compDefAccount, clusterAccount, mxeAccount, mempoolAccount, executingPool, } = getArciumAccounts(computationOffset, 'withdrawFromMixerMxe');
        const ixBuilder = program.methods
            .withdrawFromMixerMxe(convertComputationOffsetToTransactionInput(computationOffset), convertAccountOffsetToTransactionInput(publicCommissionFeesAccountOffset), convertAccountOffsetToTransactionInput(feesConfigurationAccountOffset), convertPoseidonHashToTransactionInput(txArgs.expectednullifierHash), convertPoseidonHashToTransactionInput(txArgs.expectedMerkleRoot), convertPoseidonHashToTransactionInput(txArgs.expectedLinkerAddressHash), convertGroth16ProofABeBytesToTransactionInput(txArgs.groth16ProofA), convertGroth16ProofBBeBytesToTransactionInput(txArgs.groth16ProofB), convertGroth16ProofCBeBytesToTransactionInput(txArgs.groth16ProofC), convertArciumX25519PublicKeyToTransactionInput(txArgs.ephemeralArcisPublicKey), convertArciumX25519NonceToTransactionInput(txArgs.nonce), convertRescueCiphertextToTransactionInput(txArgs.noteRecipientAddressPart1Ciphertext), convertRescueCiphertextToTransactionInput(txArgs.noteRecipientAddressPart2Ciphertext), convertRescueCiphertextToTransactionInput(txArgs.noteRecipientBlindingFactorCiphertext), convertSha3HashToTransactionInput(txArgs.noteRecipientAddressCommitment), convertAmountToTransactionInput(txArgs.amountToWithdraw), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            relayer: txAccounts.relayer,
        ...
        throw new InstructionBuildingError(`Failed to build withdraw from mixer MXE instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildWithdrawFromMixerSharedInstruction(txAccounts, txArgs) {
    try {
        const INSTRUCTION_SEED = BigInt(24);
        const computationOffset = generateRandomComputationOffset();
        const publicCommissionFeesAccountOffset = generateRandomPublicCommissionFeesAccountOffset(INSTRUCTION_SEED, txAccounts.mint);
        const feesConfigurationAccountOffset = generateRandomFeesConfigurationAccountOffset(INSTRUCTION_SEED, txAccounts.mint);
        const publicCommissionFeesAccount = getPublicCommissionFeesPoolPda(txAccounts.mint, INSTRUCTION_SEED, publicCommissionFeesAccountOffset);
        const feesConfigurationAccount = getFeesConfigurationPda(INSTRUCTION_SEED, txAccounts.mint, feesConfigurationAccountOffset);
        const { computationAccount, compDefAccount, clusterAccount, mxeAccount, mempoolAccount, executingPool, } = getArciumAccounts(computationOffset, 'withdrawFromMixerShared');
        const ixBuilder = program.methods
            .withdrawFromMixerShared(convertComputationOffsetToTransactionInput(computationOffset), convertAccountOffsetToTransactionInput(publicCommissionFeesAccountOffset), convertAccountOffsetToTransactionInput(feesConfigurationAccountOffset), convertPoseidonHashToTransactionInput(txArgs.expectednullifierHash), convertPoseidonHashToTransactionInput(txArgs.expectedMerkleRoot), convertPoseidonHashToTransactionInput(txArgs.expectedLinkerAddressHash), convertGroth16ProofABeBytesToTransactionInput(txArgs.groth16ProofA), convertGroth16ProofBBeBytesToTransactionInput(txArgs.groth16ProofB), convertGroth16ProofCBeBytesToTransactionInput(txArgs.groth16ProofC), convertArciumX25519PublicKeyToTransactionInput(txArgs.ephemeralArcisPublicKey), convertArciumX25519NonceToTransactionInput(txArgs.nonce), convertRescueCiphertextToTransactionInput(txArgs.note_creator_address_part1_ciphertext), convertRescueCiphertextToTransactionInput(txArgs.note_creator_address_part2_ciphertext), convertRescueCiphertextToTransactionInput(txArgs.blinding_factor_ciphertext), convertSha3HashToTransactionInput(txArgs.note_creator_address_commitment), convertAmountToTransactionInput(txArgs.amount_to_withdraw), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            relayer: txAccounts.relayer,
            publicCommissionFeesPool: publicCommissionFeesAccount,
            feesConfiguration: feesConfigurationAccount,
        ...
        throw new InstructionBuildingError(`Failed to build withdraw from mixer shared instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
