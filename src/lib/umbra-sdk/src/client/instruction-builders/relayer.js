...
import { convertSha3HashToTransactionInput, convertInstructionSeedToTransactionInput, convertAccountOffsetToTransactionInput, } from '@/types';
 ...
 * ```
 */
export async function buildInitialiseRelayerAccountInstruction(txAccounts, txArgs) {
    try {
        const ixBuilder = program.methods
            .initialiseRelayerAccount(convertSha3HashToTransactionInput(txArgs.endpoint))
            .accountsPartial({
        ...
        throw new InstructionBuildingError(`Failed to build initialise relayer account instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
...
export async function buildFreezeRelayerAccountInstruction(txAccounts) {
    try {
        ...
        throw new InstructionBuildingError(`Failed to build freeze relayer account instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildInitialiseRelayerFeesPoolInstruction(txAccounts, txArgs) {
    try {
        const relayerFeesPoolAccount = getRelayerFeesPoolPda(txAccounts.relayer, txArgs.instructionSeed, txAccounts.mint, txArgs.accountOffset);
        const ixBuilder = program.methods
            .initialiseRelayerFeesPool(convertInstructionSeedToTransactionInput(txArgs.instructionSeed), convertAccountOffsetToTransactionInput(txArgs.accountOffset))
            .accountsPartial({
            relayer: txAccounts.relayer,
        ...
        throw new InstructionBuildingError(`Failed to build initialise relayer fees pool instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildCollectRelayerFeesFromRelayerFeesPoolInstruction(txAccounts, txArgs) {
    try {
        const relayerFeesPoolAccount = getRelayerFeesPoolPda(txAccounts.relayer, txArgs.instructionSeed, txAccounts.mint, txArgs.accountOffset);
        const ixBuilder = program.methods
            .collectRelayerFeesFromRelayerFeesPool(convertInstructionSeedToTransactionInput(txArgs.instructionSeed), convertAccountOffsetToTransactionInput(txArgs.accountOffset))
            .accountsPartial({
        ...
        throw new InstructionBuildingError(`Failed to build collect relayer fees from relayer fees pool instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
