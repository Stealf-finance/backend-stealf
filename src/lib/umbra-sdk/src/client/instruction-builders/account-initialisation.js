...
import { convertPoseidonHashToTransactionInput, convertSha3HashToTransactionInput, } from '@/types';
 ...
 * ```
 */
export async function buildInitialiseArciumEncryptedUserAccountInstruction(txAccounts, txArgs) {
    try {
        const ixBuilder = program.methods
            .initialiseArciumEncryptedUserAccount(convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
        ...
        throw new InstructionBuildingError(`Failed to build initialise Arcium encrypted user account instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildInitialiseArciumEncryptedTokenAccountInstruction(txAccounts, txArgs) {
    try {
        const ixBuilder = program.methods
            .initialiseArciumEncryptedTokenAccount(convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            destinationAddress: txAccounts.destinationAddress,
            signer: txAccounts.signer,
        ...
        throw new InstructionBuildingError(`Failed to build initialise Arcium encrypted token account instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildInitialiseNullifierHashInstruction(txAccounts, txArgs) {
    try {
        const nullifierHashPda = getNullifierHashPda(txArgs.nullifierHash);
        const ixBuilder = program.methods
            .initialiseNullifierHash(convertPoseidonHashToTransactionInput(txArgs.nullifierHash), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
        ...
        throw new InstructionBuildingError(`Failed to build initialise nullifier hash instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
