...
import { convertInstructionSeedToTransactionInput, convertAccountOffsetToTransactionInput, convertComputationOffsetToTransactionInput, } from '@/types';
import { generateRandomComputationOffset, getArciumAccounts } from '@/utils/arcium';
import { getArciumCommissionFeesPoolPda, getPublicCommissionFeesPoolPda, } from '@/utils/pda-generators';
/**
 ...
 * ```
 */
export async function buildInitialiseCommissionFeesPoolInstruction(txAccounts, txArgs) {
    try {
        const computationOffset = generateRandomComputationOffset();
        const commissionFeesPoolAccount = getArciumCommissionFeesPoolPda(txAccounts.mint, txArgs.instructionSeed, txArgs.accountOffset);
        const { compDefAccount, clusterAccount, mxeAccount, mempoolAccount, executingPool, } = getArciumAccounts(computationOffset, 'initialise_commission_fees_pool');
        const ixBuilder = program.methods
            .initialiseCommissionFeesPool(convertComputationOffsetToTransactionInput(computationOffset), convertInstructionSeedToTransactionInput(txArgs.instructionSeed), convertAccountOffsetToTransactionInput(txArgs.accountOffset))
            .accountsPartial({
        ...
        throw new InstructionBuildingError(`Failed to build initialise commission fees pool instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildCollectCommissionFeesFromCommissionFeesPoolInstruction(txAccounts, txArgs) {
    try {
        const computationOffset = generateRandomComputationOffset();
        const commissionFeesPoolAccount = getArciumCommissionFeesPoolPda(txAccounts.mint, txArgs.instructionSeed, txArgs.accountOffset);
        const { compDefAccount, clusterAccount, mxeAccount, mempoolAccount, executingPool, } = getArciumAccounts(computationOffset, 'collect_commission_fees_from_commission_fees_pool');
        const ixBuilder = program.methods
            .collectCommissionFeesFromCommissionFeesPool(convertComputationOffsetToTransactionInput(computationOffset), convertInstructionSeedToTransactionInput(txArgs.instructionSeed), convertAccountOffsetToTransactionInput(txArgs.accountOffset))
            .accountsPartial({
            payer: txAccounts.payer,
        ...
        throw new InstructionBuildingError(`Failed to build collect commission fees from commission fees pool instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildInitialisePublicCommissionFeesPoolInstruction(txAccounts, txArgs) {
    try {
        const publicCommissionFeesPoolAccount = getPublicCommissionFeesPoolPda(txAccounts.mint, txArgs.instructionSeed, txArgs.accountOffset);
        const ixBuilder = program.methods
            .initialisePublicCommissionFees(convertInstructionSeedToTransactionInput(txArgs.instructionSeed), convertAccountOffsetToTransactionInput(txArgs.accountOffset))
            .accountsPartial({
        ...
        throw new InstructionBuildingError(`Failed to build initialise public commission fees pool instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
