...
import { convertRescueCiphertextToTransactionInput, convertPoseidonHashToTransactionInput, convertGroth16ProofCBeBytesToTransactionInput, convertAccountOffsetToTransactionInput, convertSha3HashToTransactionInput, convertGroth16ProofABeBytesToTransactionInput, convertGroth16ProofBBeBytesToTransactionInput, convertArciumX25519NonceToTransactionInput, convertArciumX25519PublicKeyToTransactionInput, convertComputationOffsetToTransactionInput, convertAmountToTransactionInput, convertTimeToTransactionInput, } from '@/types';
import { generateRandomComputationOffset, getArciumAccounts } from '@/utils/arcium';
import { generateRandomArciumCommissionFeesAccountOffset, generateRandomFeesConfigurationAccountOffset, generateRandomPublicCommissionFeesAccountOffset, generateRandomRelayerFeesPoolOffset, } from '@/utils/miscellaneous';
import { getArciumCommissionFeesPoolPda, getFeesConfigurationPda, getNullifierHashPda, getPublicCommissionFeesPoolPda, getRelayerFeesPoolPda, } from '@/utils/pda-generators';
/**
 ...
 * ```
 */
export async function buildNewTokenDepositMxeInstruction(txAccounts, txArgs) {
    try {
        const INSTRUCTION_SEED = BigInt(7);
        const computationOffset = generateRandomComputationOffset();
        const commissionFeesAccountOffset = generateRandomArciumCommissionFeesAccountOffset(INSTRUCTION_SEED, txAccounts.mint);
        const feesConfigurationAccountOffset = generateRandomFeesConfigurationAccountOffset(INSTRUCTION_SEED, txAccounts.mint);
        const commissionFeePoolAccount = getArciumCommissionFeesPoolPda(txAccounts.mint, INSTRUCTION_SEED, commissionFeesAccountOffset);
        ...
        const { compDefAccount, clusterAccount, mxeAccount, mempoolAccount, executingPool, } = getArciumAccounts(computationOffset, 'new_token_deposit_mxe');
        const ixBuilder = program.methods
            .newTokenDepositMxe(convertComputationOffsetToTransactionInput(computationOffset), convertAccountOffsetToTransactionInput(commissionFeesAccountOffset), convertAccountOffsetToTransactionInput(feesConfigurationAccountOffset), convertPoseidonHashToTransactionInput(txArgs.expectedNullifierHash), convertArciumX25519PublicKeyToTransactionInput(txArgs.ephemeralArcisPublicKey), convertArciumX25519NonceToTransactionInput(txArgs.nonce), convertRescueCiphertextToTransactionInput(txArgs.depositAmountCiphertext), convertRescueCiphertextToTransactionInput(txArgs.depositorAddressPart1Ciphertext), convertRescueCiphertextToTransactionInput(txArgs.depositorAddressPart2Ciphertext), convertRescueCiphertextToTransactionInput(txArgs.blindingFactor), convertSha3HashToTransactionInput(txArgs.commitment), convertGroth16ProofABeBytesToTransactionInput(txArgs.groth16ProofA), convertGroth16ProofBBeBytesToTransactionInput(txArgs.groth16ProofB), convertGroth16ProofCBeBytesToTransactionInput(txArgs.groth16ProofC), convertPoseidonHashToTransactionInput(txArgs.expectedMerkleRoot), convertPoseidonHashToTransactionInput(txArgs.expectedLinkerAddressHash), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
        ...
        throw new InstructionBuildingError(`Failed to build new token deposit MXE instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildExistingTokenDepositMxeInstruction(txAccounts, txArgs) {
    try {
        const INSTRUCTION_SEED = BigInt(8);
        const computationOffset = generateRandomComputationOffset();
        const commissionFeesAccountOffset = generateRandomArciumCommissionFeesAccountOffset(INSTRUCTION_SEED, txAccounts.mint);
        const feesConfigurationAccountOffset = generateRandomFeesConfigurationAccountOffset(INSTRUCTION_SEED, txAccounts.mint);
        const commissionFeePoolAccount = getArciumCommissionFeesPoolPda(txAccounts.mint, INSTRUCTION_SEED, commissionFeesAccountOffset);
        const feesConfigurationAccount = getFeesConfigurationPda(INSTRUCTION_SEED, txAccounts.mint, feesConfigurationAccountOffset);
        const nullifierHashAccount = getNullifierHashPda(txArgs.expectedNullifierHash);
        const { compDefAccount, clusterAccount, mxeAccount, mempoolAccount, executingPool, } = getArciumAccounts(computationOffset, 'existing_token_deposit_mxe');
        const ixBuilder = program.methods
            .existingTokenDepositMxe(convertComputationOffsetToTransactionInput(computationOffset), convertAccountOffsetToTransactionInput(commissionFeesAccountOffset), convertAccountOffsetToTransactionInput(feesConfigurationAccountOffset), convertPoseidonHashToTransactionInput(txArgs.expectedNullifierHash), convertArciumX25519PublicKeyToTransactionInput(txArgs.ephemeralArcisPublicKey), convertArciumX25519NonceToTransactionInput(txArgs.nonce), convertRescueCiphertextToTransactionInput(txArgs.depositAmountCiphertext), convertRescueCiphertextToTransactionInput(txArgs.depositorAddressPart1Ciphertext), convertRescueCiphertextToTransactionInput(txArgs.depositorAddressPart2Ciphertext), convertRescueCiphertextToTransactionInput(txArgs.blindingFactor), convertSha3HashToTransactionInput(txArgs.commitment), convertGroth16ProofABeBytesToTransactionInput(txArgs.groth16ProofA), convertGroth16ProofBBeBytesToTransactionInput(txArgs.groth16ProofB), convertGroth16ProofCBeBytesToTransactionInput(txArgs.groth16ProofC), convertPoseidonHashToTransactionInput(txArgs.expectedMerkleRoot), convertPoseidonHashToTransactionInput(txArgs.expectedLinkerAddressHash), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            relayer: txAccounts.relayer,
            accountAddress: txAccounts.destinationAddress,
            mint: txAccounts.mint,
        ...
        throw new InstructionBuildingError(`Failed to build existing token deposit MXE instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildNewTokenDepositSharedInstruction(txAccounts, txArgs) {
    try {
        const INSTRUCTION_SEED = BigInt(9);
        const computationOffset = generateRandomComputationOffset();
        const commissionFeesAccountOffset = generateRandomArciumCommissionFeesAccountOffset(INSTRUCTION_SEED, txAccounts.mint);
        const feesConfigurationAccountOffset = generateRandomFeesConfigurationAccountOffset(INSTRUCTION_SEED, txAccounts.mint);
        const commissionFeePoolAccount = getArciumCommissionFeesPoolPda(txAccounts.mint, INSTRUCTION_SEED, commissionFeesAccountOffset);
        const feesConfigurationAccount = getFeesConfigurationPda(INSTRUCTION_SEED, txAccounts.mint, feesConfigurationAccountOffset);
        const nullifierHashAccount = getNullifierHashPda(txArgs.expectedNullifierHash);
        const { compDefAccount, clusterAccount, mxeAccount, mempoolAccount, executingPool, } = getArciumAccounts(computationOffset, 'new_token_deposit_shared');
        const ixBuilder = program.methods
            .newTokenDepositShared(convertComputationOffsetToTransactionInput(computationOffset), convertAccountOffsetToTransactionInput(commissionFeesAccountOffset), convertAccountOffsetToTransactionInput(feesConfigurationAccountOffset), convertPoseidonHashToTransactionInput(txArgs.expectedNullifierHash), convertArciumX25519PublicKeyToTransactionInput(txArgs.ephemeralArcisPublicKey), convertRescueCiphertextToTransactionInput(txArgs.depositAmountCiphertext), convertRescueCiphertextToTransactionInput(txArgs.depositorAddressPart1Ciphertext), convertRescueCiphertextToTransactionInput(txArgs.depositorAddressPart2Ciphertext), convertRescueCiphertextToTransactionInput(txArgs.blindingFactor), convertArciumX25519NonceToTransactionInput(txArgs.nonce), convertSha3HashToTransactionInput(txArgs.commitment), convertGroth16ProofABeBytesToTransactionInput(txArgs.groth16ProofA), convertGroth16ProofBBeBytesToTransactionInput(txArgs.groth16ProofB), convertGroth16ProofCBeBytesToTransactionInput(txArgs.groth16ProofC), convertPoseidonHashToTransactionInput(txArgs.expectedMerkleRoot), convertPoseidonHashToTransactionInput(txArgs.expectedLinkerAddressHash), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            relayer: txAccounts.relayer,
            accountAddress: txAccounts.destinationAddress,
            mint: txAccounts.mint,
        ...
        throw new InstructionBuildingError(`Failed to build new token deposit shared instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildExistingTokenDepositSharedInstruction(txAccounts, txArgs) {
    try {
        const INSTRUCTION_SEED = BigInt(10);
        const computationOffset = generateRandomComputationOffset();
        const commissionFeesAccountOffset = generateRandomArciumCommissionFeesAccountOffset(INSTRUCTION_SEED, txAccounts.mint);
        const feesConfigurationAccountOffset = generateRandomFeesConfigurationAccountOffset(INSTRUCTION_SEED, txAccounts.mint);
        const commissionFeePoolAccount = getArciumCommissionFeesPoolPda(txAccounts.mint, INSTRUCTION_SEED, commissionFeesAccountOffset);
        const feesConfigurationAccount = getFeesConfigurationPda(INSTRUCTION_SEED, txAccounts.mint, feesConfigurationAccountOffset);
        const nullifierHashAccount = getNullifierHashPda(txArgs.expectedNullifierHash);
        const { compDefAccount, clusterAccount, mxeAccount, mempoolAccount, executingPool, } = getArciumAccounts(computationOffset, 'existing_token_deposit_shared');
        const ixBuilder = program.methods
            .existingTokenDepositShared(convertComputationOffsetToTransactionInput(computationOffset), convertAccountOffsetToTransactionInput(commissionFeesAccountOffset), convertAccountOffsetToTransactionInput(feesConfigurationAccountOffset), convertPoseidonHashToTransactionInput(txArgs.expectedNullifierHash), convertArciumX25519PublicKeyToTransactionInput(txArgs.ephemeralArcisPublicKey), convertRescueCiphertextToTransactionInput(txArgs.depositAmountCiphertext), convertRescueCiphertextToTransactionInput(txArgs.depositorAddressPart1Ciphertext), convertRescueCiphertextToTransactionInput(txArgs.depositorAddressPart2Ciphertext), convertRescueCiphertextToTransactionInput(txArgs.blindingFactor), convertArciumX25519NonceToTransactionInput(txArgs.nonce), convertSha3HashToTransactionInput(txArgs.commitment), convertGroth16ProofABeBytesToTransactionInput(txArgs.groth16ProofA), convertGroth16ProofBBeBytesToTransactionInput(txArgs.groth16ProofB), convertGroth16ProofCBeBytesToTransactionInput(txArgs.groth16ProofC), convertPoseidonHashToTransactionInput(txArgs.expectedMerkleRoot), convertPoseidonHashToTransactionInput(txArgs.expectedLinkerAddressHash), convertSha3HashToTransactionInput(txArgs.optionalData))
            .accountsPartial({
            relayer: txAccounts.relayer,
            accountAddress: txAccounts.destinationAddress,
            mint: txAccounts.mint,
        ...
        throw new InstructionBuildingError(`Failed to build existing token deposit shared instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildDepositIntoMixerSolInstruction(txAccounts, txArgs) {
    try {
        const INSTRUCTION_SEED = BigInt(19);
        const relayerOffset = generateRandomRelayerFeesPoolOffset(INSTRUCTION_SEED, WSOL_MINT_ADDRESS);
        const commissionFeesAccountOffset = generateRandomPublicCommissionFeesAccountOffset(INSTRUCTION_SEED, WSOL_MINT_ADDRESS);
        const feesConfigurationAccountOffset = generateRandomFeesConfigurationAccountOffset(INSTRUCTION_SEED, WSOL_MINT_ADDRESS);
        const relayerFeesPoolAccount = getRelayerFeesPoolPda(txAccounts.relayer, INSTRUCTION_SEED, WSOL_MINT_ADDRESS, relayerOffset);
        const publicCommissionFeesPoolAccount = getPublicCommissionFeesPoolPda(WSOL_MINT_ADDRESS, INSTRUCTION_SEED, commissionFeesAccountOffset);
        const feesConfigurationAccount = getFeesConfigurationPda(INSTRUCTION_SEED, WSOL_MINT_ADDRESS, feesConfigurationAccountOffset);
        const ixBuilder = program.methods
            .depositIntoMixerPoolSol(convertAccountOffsetToTransactionInput(relayerOffset), convertAccountOffsetToTransactionInput(commissionFeesAccountOffset), convertAccountOffsetToTransactionInput(feesConfigurationAccountOffset), convertAmountToTransactionInput(txArgs.amount), convertPoseidonHashToTransactionInput(txArgs.commitmentInnerHash), convertPoseidonHashToTransactionInput(txArgs.linkerHash), convertTimeToTransactionInput(txArgs.time), convertGroth16ProofABeBytesToTransactionInput(txArgs.groth16ProofA), convertGroth16ProofBBeBytesToTransactionInput(txArgs.groth16ProofB), convertGroth16ProofCBeBytesToTransactionInput(txArgs.groth16ProofC))
            .accountsPartial({
        ...
        throw new InstructionBuildingError(`Failed to build deposit into mixer pool SOL instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export async function buildDepositIntoMixerPoolSplInstruction(txAccounts, txArgs) {
    try {
        const INSTRUCTION_SEED = BigInt(20);
        const relayerOffset = generateRandomRelayerFeesPoolOffset(INSTRUCTION_SEED, WSOL_MINT_ADDRESS);
        const commissionFeesAccountOffset = generateRandomPublicCommissionFeesAccountOffset(INSTRUCTION_SEED, WSOL_MINT_ADDRESS);
        const feesConfigurationAccountOffset = generateRandomFeesConfigurationAccountOffset(INSTRUCTION_SEED, WSOL_MINT_ADDRESS);
        const relayerFeesPoolAccount = getRelayerFeesPoolPda(txAccounts.relayer, INSTRUCTION_SEED, WSOL_MINT_ADDRESS, relayerOffset);
        ...
        const feesConfigurationAccount = getFeesConfigurationPda(INSTRUCTION_SEED, WSOL_MINT_ADDRESS, feesConfigurationAccountOffset);
        const ixBuilder = program.methods
            .depositIntoMixerPoolSpl(convertAccountOffsetToTransactionInput(relayerOffset), convertAccountOffsetToTransactionInput(commissionFeesAccountOffset), convertAccountOffsetToTransactionInput(feesConfigurationAccountOffset), convertAmountToTransactionInput(txArgs.amount), convertPoseidonHashToTransactionInput(txArgs.commitmentInnerHash), convertPoseidonHashToTransactionInput(txArgs.linkerHash), convertTimeToTransactionInput(txArgs.time), convertGroth16ProofABeBytesToTransactionInput(txArgs.groth16ProofA), convertGroth16ProofBBeBytesToTransactionInput(txArgs.groth16ProofB), convertGroth16ProofCBeBytesToTransactionInput(txArgs.groth16ProofC))
            .accountsPartial({
            arciumSigner: txAccounts.arciumSigner,
            relayer: txAccounts.relayer,
            relayerFeesPool: relayerFeesPoolAccount,
        ...
        throw new InstructionBuildingError(`Failed to build deposit into mixer pool SPL instruction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
