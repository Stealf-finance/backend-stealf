...
import { convertU128LeBytesToU128, convertU16LeBytesToU16, convertU256LeBytesToU256, } from './convertors';
 ...
 * ```
 */
export function convertLegacyTransactionToVersionedTransaction(transaction) {
    if (!transaction.feePayer) {
        throw new TransactionConversionError('Transaction must have a fee payer set to convert to VersionedTransaction');
    }
    if (!transaction.recentBlockhash) {
        throw new TransactionConversionError('Transaction must have a recent blockhash set to convert to VersionedTransaction');
    }
    try {
        ...
        throw new TransactionConversionError(`Failed to convert transaction to VersionedTransaction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export function generateRandomRelayerFeesPoolOffset(_instructionSeed, _mintAddress) {
    ...
    try {
        ...
        const randomU16 = convertU16LeBytesToU16(randomBytesArray);
        const randomNumber = Number(randomU16) % NUMBER_OF_ACCOUNTS;
        return BigInt(randomNumber);
    }
    catch (error) {
        throw new AccountOffsetGenerationError(`Failed to generate random relayer fees pool offset: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export function generateRandomFeesConfigurationAccountOffset(_instructionSeed, _mintAddress) {
    const NUMBER_OF_ACCOUNTS = 1;
    if (NUMBER_OF_ACCOUNTS <= 0) {
        throw new AccountOffsetGenerationError('Number of accounts must be greater than 0');
    }
    try {
        ...
        throw new AccountOffsetGenerationError(`Failed to generate random fees configuration account offset: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export function generateRandomArciumCommissionFeesAccountOffset(_instructionSeed, _mintAddress) {
    const NUMBER_OF_ACCOUNTS = 1;
    if (NUMBER_OF_ACCOUNTS <= 0) {
        throw new AccountOffsetGenerationError('Number of accounts must be greater than 0');
    }
    try {
        ...
        throw new AccountOffsetGenerationError(`Failed to generate random Arcium commission fees account offset: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export function generateRandomPublicCommissionFeesAccountOffset(_instructionSeed, _mintAddress) {
    const NUMBER_OF_ACCOUNTS = 1;
    if (NUMBER_OF_ACCOUNTS <= 0) {
        throw new AccountOffsetGenerationError('Number of accounts must be greater than 0');
    }
    try {
        ...
        throw new AccountOffsetGenerationError(`Failed to generate random public commission fees account offset: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 */
export function isBitSet(value, bit) {
 ...
 * ```
 */
export function generateRandomU256() {
    const randomBytesArray = randomBytes(32);
    return convertU256LeBytesToU256(randomBytesArray);
}
/**
 ...
 * ```
 */
export function generateRandomBlindingFactor() {
    const randomBytesArray = randomBytes(16);
    return convertU128LeBytesToU128(randomBytesArray);
}
/**
 ...
 * ```
 */
export function breakPublicKeyIntoTwoParts(publicKey) {
    ...
    return [
        convertU128LeBytesToU128(publicKeyBytesFirstHalf),
        convertU128LeBytesToU128(publicKeyBytesSecondHalf),
    ];
}
/**
 ...
 */
export function getSiblingMerkleIndicesFromInsertionIndex(insertionIndex) {
    if (insertionIndex < BigInt(0)) {
        throw new RangeError('Insertion index cannot be negative when computing Merkle siblings.');
    }
    const maxInsertionIndex = BigInt(1) << BigInt(MERKLE_TREE_DEPTH);
    if (insertionIndex >= maxInsertionIndex) {
        throw new RangeError(`Insertion index ${insertionIndex.toString()} exceeds tree capacity for depth ${MERKLE_TREE_DEPTH}.`);
    }
    const siblingIndices = [];
    for (let i = 0; i < MERKLE_TREE_DEPTH; ++i) {
        siblingIndices.push((insertionIndex & (BigInt(1) << BigInt(i))) === BigInt(0) ? 1 : 0);
    }
    return siblingIndices;
}
export function convertHexStringToPoseidonHash(hexString) {
