...
import { convertLeBytesToBuffer, convertU128ToLeBytes, convertU16ToLeBytes, convertU32ToLeBytes, } from '@/utils/convertors';
import { program } from '@/idl';
import { NULLIFIER_HASH_PDA_SEED, USER_ALLOTED_COMPLIANCE_GRANT_PDA_SEED, } from '@/constants/anchor';
/**
 ...
 */
export function getNullifierHashPda(nullifierHash) {
    try {
        ...
        throw new PdaGenerationError('Failed to derive nullifier hash PDA', error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * @throws {@link PdaGenerationError} When PDA derivation fails
 */
export function getUserAllotedComplianceGrantPda(senderPublicKey, destinationPublicKey, nonce) {
    try {
        return PublicKey.findProgramAddressSync([
            Buffer.from(USER_ALLOTED_COMPLIANCE_GRANT_PDA_SEED),
            senderPublicKey.toBuffer(),
            destinationPublicKey.toBuffer(),
            convertLeBytesToBuffer(convertU128ToLeBytes(nonce)),
        ], program.programId)[0];
    }
    catch (error) {
        throw new PdaGenerationError('Failed to derive user-allotted compliance grant PDA', error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * @throws {@link PdaGenerationError} When PDA derivation fails
 */
export function getRelayerFeesPoolPda(relayer, instructionSeed, mint, accountOffset) {
    try {
        return PublicKey.findProgramAddressSync([
            Buffer.from('relayer_fees_pool:'),
            convertLeBytesToBuffer(convertU16ToLeBytes(instructionSeed)),
            relayer.toBuffer(),
            mint.toBuffer(),
            convertLeBytesToBuffer(convertU16ToLeBytes(accountOffset)),
        ], program.programId)[0];
    }
    catch (error) {
        throw new PdaGenerationError('Failed to derive relayer fees pool PDA', error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * @throws {@link PdaGenerationError} When PDA derivation fails
 */
export function getFeesConfigurationPda(instructionSeed, mint, accountOffset) {
    try {
        return PublicKey.findProgramAddressSync([
        ...
        throw new PdaGenerationError('Failed to derive fees configuration PDA', error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * @throws {@link PdaGenerationError} When PDA derivation fails
 */
export function getArciumCommissionFeesPoolPda(mint, instructionSeed, accountOffset) {
    try {
        return PublicKey.findProgramAddressSync([
        ...
        throw new PdaGenerationError('Failed to derive Arcium commission fees pool PDA', error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * @throws {@link PdaGenerationError} When PDA derivation fails
 */
export function getPublicCommissionFeesPoolPda(mint, instructionSeed, accountOffset) {
    try {
        return PublicKey.findProgramAddressSync([
        ...
        throw new PdaGenerationError('Failed to derive public commission fees pool PDA', error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * @throws {@link PdaGenerationError} When PDA derivation fails
 */
export function getWalletSpecifierPda(instructionSeed) {
    try {
        return PublicKey.findProgramAddressSync([
            Buffer.from('wallet_specifier:'),
            convertLeBytesToBuffer(convertU32ToLeBytes(instructionSeed)),
        ], program.programId)[0];
    }
    catch (error) {
        throw new PdaGenerationError('Failed to derive wallet specifier PDA', error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * @throws {@link PdaGenerationError} When PDA derivation fails
 */
export function getArciumEncryptedUserAccountPda(userPublicKey) {
    try {
        return PublicKey.findProgramAddressSync([Buffer.from('arcium_encrypted_user_account'), userPublicKey.toBuffer()], program.programId)[0];
    }
    catch (error) {
        throw new PdaGenerationError('Failed to derive Arcium-encrypted user account PDA', error instanceof Error ? error : undefined);
    }
}
/**
 ...
 */
export function getArciumEncryptedTokenAccountPda(userPublicKey, mint) {
    try {
        return PublicKey.findProgramAddressSync([
        ...
        throw new PdaGenerationError('Failed to derive Arcium-encrypted token account PDA', error instanceof Error ? error : undefined);
    }
}
