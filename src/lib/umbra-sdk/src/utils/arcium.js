import { getArciumAccountBaseSeed, getArciumProgAddress, getCompDefAccAddress, getCompDefAccOffset, getComputationAccAddress, getMXEAccAddress, } from '@arcium-hq/client';
import { PublicKey } from '@solana/web3.js';
import { program } from '@/idl';
import { randomBytes } from '@noble/hashes/utils.js';
import { convertU32LeBytesToU32, convertU64LeBytesToU64 } from './convertors';
import { BN } from 'bn.js';
import { ARCIUM_CLUSTER_ACCOUNT, ARCIUM_MXE_ACCOUNT, ARCIUM_MEMPOOL_ACCOUNT, ARCIUM_EXECUTING_POOL_ACCOUNT, } from '@/constants/arcium';
/**
 ...
 * ```
 */
export function getCompDefInitialisationTransactionBuilder(compDefName) {
    ...
    return async (owner) => {
        try {
            ...
            const compDefPDA = PublicKey.findProgramAddressSync([baseSeedCompDefAcc, program.programId.toBuffer(), offset], getArciumProgAddress())[0];
            const methods = program.methods;
            const method = methods[methodName];
            if (!method) {
                throw new CompDefInitialisationError(`Program method '${methodName}' not found. Make sure the computation definition name '${compDefName}' is correct and the corresponding initialization method exists in the program.`);
            }
            ...
            if (!transaction) {
                throw new CompDefInitialisationError('Transaction builder returned null or undefined');
            }
            return transaction;
        }
        catch (error) {
            ...
            throw new CompDefInitialisationError(`Failed to build computation definition initialization transaction: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
        }
    };
}
/**
 ...
 * ```
 */
export function generateRandomComputationOffset() {
    try {
        const randomComputationOffsetBytes = randomBytes(8);
        return convertU64LeBytesToU64(randomComputationOffsetBytes);
    }
    catch (error) {
        throw new ArciumAccountError(`Failed to generate random computation offset: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
/**
 ...
 * ```
 */
export function getArciumAccounts(computationOffset, computationName) {
    if (!computationName || computationName.trim().length === 0) {
        throw new ArciumAccountError('Computation name cannot be empty');
    }
    try {
        const computationAccount = getComputationAccAddress(program.programId, new BN(computationOffset));
        if (!computationAccount) {
            throw new ArciumAccountError('Failed to derive computation account address');
        }
        const compDefOffset = getCompDefAccOffset(computationName);
        const compDefOffsetU32 = convertU32LeBytesToU32(compDefOffset);
        const compDefAccount = getCompDefAccAddress(program.programId, Number(compDefOffsetU32));
        if (!compDefAccount) {
            throw new ArciumAccountError(`Failed to derive computation definition account address for '${computationName}'`);
        }
        ...
        if (!clusterAccount || !mxeAccount || !mempoolAccount || !executingPool) {
            throw new ArciumAccountError('One or more Arcium constant accounts are undefined');
        }
        return {
        ...
        throw new ArciumAccountError(`Failed to get Arcium accounts: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    }
}
