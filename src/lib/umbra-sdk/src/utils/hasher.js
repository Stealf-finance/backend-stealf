 ...
 * @public
 */
export class PoseidonHasherError extends Error {
    /**
...
function parseConstants(value) {
    ...
    if (value && typeof value === 'object') {
        return Object.fromEntries(Object.entries(value).map(([k, v]) => [k, parseConstants(v)]));
    }
    return value;
}
/**
...
function getCircomParameters(t) {
    ...
    if (index < 0 || index >= ROUNDS_PARTIAL.length) {
        throw new PoseidonParameterError(`Unsupported t=${t}. Circom constants only cover t from 2 to ${ROUNDS_PARTIAL.length + 1}.`);
    }
    const roundsPartial = ROUNDS_PARTIAL[index];
    const parsedConstants = parseConstants(POSEIDON_CONSTANTS);
...
export class PoseidonHasher {
     ...
     * @internal
     */
    static getPoseidonInstance(inputSize) {
    ...
    static hash(inputs) {
        if (inputs.length === 0) {
            throw new PoseidonInputError('Poseidon hash requires at least 1 input element');
        }
        if (inputs.length > 17) {
            throw new PoseidonInputError(`Poseidon hash supports at most 17 input elements, got ${inputs.length}`);
        }
        const poseidon = this.getPoseidonInstance(inputs.length);
        const initState = 0n;
        const state = poseidon([initState, ...inputs]);
        const hashValue = state[0];
        return convertBigIntToLeBytes(hashValue, 32);
    }
}
PoseidonHasher.poseidonInstances = new Map();
/**
 ...
 * @internal
 */
function aggregatePoseidonHashes(hashes) {
 ...
 */
export function aggregateSha3HashIntoSinglePoseidonRoot(sha3Hash) {
    if (!(sha3Hash instanceof Uint8Array) || sha3Hash.length !== 32) {
        throw new TypeError('aggregateSha3HashIntoSinglePoseidonRoot expects a 32-byte Uint8Array.');
    }
    ...
    const groupSizes = [...Array(21).fill(12), 4];
    ...
    if (offset !== 256) {
        throw new PoseidonInputError(`Internal error: expected to consume 256 bits, consumed ${offset}`);
    }
