...
class SimpleMixerService {
  ...
  async deposit(params: {
    ...
    if (MIXER_CONFIG.ENABLE_STANDARDIZED_POOLS) {
      ...
      if (!isStandardAmount) {
        throw new Error(
          `Standardized pools enabled. Amount must be one of: ${MIXER_CONFIG.STANDARD_POOL_SIZES.map(a => a / LAMPORTS_PER_SOL).join(', ')} SOL`
        );
      }
    }
    ...
    const estimatedWithdrawTime = new Date(
      Date.now() + MIXER_CONFIG.MIN_WITHDRAWAL_DELAY
    );

    return {
      claimSecret, // USER MUST SAVE THIS
      depositTxSignature: signature,
  ...
  async getStats(): Promise<{
    poolStats: Awaited<ReturnType<typeof poolManagerService.getPoolStats>>;
    totalDeposits: number;
    unclaimedDeposits: number;
    claimedDeposits: number;
   ...
   */
  async getDepositStatus(claimSecret: string): Promise<{
    exists: boolean;
    claimed: boolean;
    amount?: number;
    amountSOL?: number;
   ...
   */
  private generateClaimSecret(): string {
   ...
   */
  private hashClaimSecret(secret: string): string {
