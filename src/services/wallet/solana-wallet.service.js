...
class SolanaWalletService {
     ...
     */
    async getKeypairForUser(userId) {
        ...
        if (user.solanaWallet && user.encryptedPrivateKey) {
            try {
                const keypair = this.decryptKeypair(user.encryptedPrivateKey);
     ...
     */
    async getKeypairFromPrivateKey(privateKeyBase58) {
     ...
     */
    async generateDeterministicKeypair(userId, userSecret) {
        // Derive seed from user ID + secret
        const seed = crypto.pbkdf2Sync(userId + userSecret, 'solana-wallet-seed', 100000, 32, 'sha256');
     ...
     */
    async importPrivateKey(userId, privateKeyBase58) {
