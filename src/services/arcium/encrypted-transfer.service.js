...
import { RescueCipher, getComputationAccAddress, getMXEAccAddress, getMempoolAccAddress, getExecutingPoolAccAddress, getClusterAccAddress, getCompDefAccAddress, } from '@arcium-hq/client';
...
class EncryptedTransferService {
     ...
     */
    async initializeCompDef(payerKeypair) {
        if (!this.connection) {
    ...
    async createEncryptedTransfer(params) {
        ...
        // Step 5: Derive PDAs
        const [transferAccount] = PublicKey.findProgramAddressSync([TRANSFER_SEED, fromKeypair.publicKey.toBuffer(), computationOffset.toArrayLike(Buffer, 'le', 8)], PROGRAM_ID);
        const [signPdaAccount] = PublicKey.findProgramAddressSync([SIGNER_ACCOUNT_SEED], PROGRAM_ID);
        // Get Arcium accounts
        const mxeAccount = getMXEAccAddress(PROGRAM_ID);
        ...
        let signature;
        try {
            signature = await this.connection.sendTransaction(transaction, [fromKeypair], { skipPreflight: false, preflightCommitment: 'confirmed' });
            await this.connection.confirmTransaction(signature, 'confirmed');
     ...
     */
    async getTransferStatus(sender, computationOffset) {
        if (!this.connection) {
            throw new Error('Service not initialized');
        }
        const [transferAccount] = PublicKey.findProgramAddressSync([TRANSFER_SEED, sender.toBuffer(), computationOffset.toArrayLike(Buffer, 'le', 8)], PROGRAM_ID);
     ...
     */
    async waitForCompletion(sender, computationOffset, timeoutMs = 60000) {
