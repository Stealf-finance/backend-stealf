...
import { accountInitService } from './account-init.service.js';
...
interface DepositConfidentialParams {
  userId: string;
  keypair: Keypair;
  amount: bigint;
  mint?: string;
...
class DepositService {
   ...
   */
  async depositPublic(params: DepositPublicParams): Promise<DepositResult> {
    ...
    try {
      ...
      try {
        ...
        await client.registerAccountForAnonymity(ZERO_SHA3_HASH as any, { mode: 'connection' });
      ...
      const result = await client.depositPubliclyIntoMixerPool(
        amount as any, // Cast to Amount type
        userPubkeyForDebug as any,
        mintAddress as any, // Pass the exact WSOL_MINT_ADDRESS instance or new PublicKey
        {
          mode: 'connection',
          generationIndex: generationIndex as any,
          relayerPublicKey: userPubkeyForDebug as any, // Use own address as relayer to avoid fetching random relayer
        }
      );
      ...
      const transaction = await Transaction.create({
        ...
        type: 'deposit',
        status: 'confirmed',
        signature: result.txReturnedData as string,
        mint,
   ...
   */
  async depositConfidential(params: DepositConfidentialParams): Promise<DepositResult> {
    ...
    try {
      ...
      const result = await client.depositConfidentiallyIntoMixerPool(
        amount as any,
        await wallet.signer.getPublicKey() as any,
        mint as any,
        relayer as any,
        {
      ...
      const transaction = await Transaction.create({
        userId,
        type: 'deposit',
        status: 'pending', // Will be confirmed by relayer
        mint,
        amount: amount.toString(),
        claimableBalance: result.claimableBalance.toString(),
        generationIndex: result.generationIndex.toString(),
   ...
   */
  async getClaimableDeposits(userId: string): Promise<any[]> {
