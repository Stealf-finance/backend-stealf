...
class ClaimService {
   ...
   */
  async claimDeposit(params: ClaimParams): Promise<ClaimResult> {
    ...
    try {
      ...
      const result = await client.claimDepositFromMixerPool(
        BigInt(depositArtifacts.generationIndex) as any,
        BigInt(depositArtifacts.time) as any,
        recipient as any,
        depositArtifacts.mint as any,
        new PublicKey(depositArtifacts.relayerPublicKey) as any,
        {
          mode: 'connection',
        }
      );
   ...
   */
  async getClaimedDeposits(userId: string): Promise<any[]> {
   ...
   */
  async getClaimHistory(userId: string): Promise<any[]> {
