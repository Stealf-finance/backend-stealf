...
import type { Sha3Hash, SolanaAddress } from '../../lib/umbra-sdk/dist/index.mjs';
...
class AccountInitService {
   ...
   */
  async initializeArciumUserAccount(userId: string, keypair: Keypair): Promise<{ signature: string; pda: string }> {
    try {
      ...
      const instructionBuilder = await AccountInitialisationInstructions.buildInitialiseArciumEncryptedUserAccountInstruction(
        {
          destinationAddress: userPubkey as SolanaAddress,
          signer: userPubkey as SolanaAddress,
        },
        {
      ...
      console.log('📋 Instruction accounts:', JSON.stringify(
      ...
      const solanaKeypair = (wallet.signer as KeypairSigner).getKeypair();
   ...
   */
  async isAccountInitialized(userId: string, keypair: Keypair): Promise<boolean> {
    try {
      const wallet = await umbraWalletService.getOrCreateWallet(userId, keypair);
      const userPubkey = await wallet.signer.getPublicKey();
      const connection = umbraClientService.getConnection();
